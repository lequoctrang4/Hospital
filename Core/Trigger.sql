...
-- Xóa bệnh nhân thì xóa luôn hồ sơ bệnh nhân
CREATE OR REPLACE TRIGGER INPATIENT_TREAT
    BEFORE
    DELETE
    ON PATIENT
    FOR EACH ROW
BEGIN
    DELETE FROM TREAT WHERE TREAT.PATIENT_ID = :OLD.PATIENT_ID;
END INPATIENT_TREAT;

/
DROP TRIGGER INPATIENT_TREAT;
DELETE FROM TREAT WHERE PATIENT_ID = 10;
/
